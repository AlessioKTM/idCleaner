...
class InboxFolderCleaner(BaseFolderCleaner):
    ...
    def _bucking(self, mailbox):
        ...
        # --- Phase 1: Light Triage (Only Headers) ---
        for msg in mailbox.fetch(reverse=True, headers_only=True):
            ...
            if msg.from_ in blacklist:
                if has_unsubscribe_header(msg):
                    ...
                    self.record.write(f"Blacklist: Found unsubscribe header for {msg.from_}.")
                categories["to_bin"].append(msg)
                self.record.write(f"Blacklist: Sender {msg.from_} is blacklisted. Moving to trash.")
                continue

            if msg.from_ not in whitelist and msg.from_ not in graylist:
                categories["to_graylist"].append(msg)
                self.record.write(f"New Sender: Found unknown sender {msg.from_}. Moving to graylist.")
                continue

            uids_to_fetch.append(msg.uid)

        # --- Phase 2: Deep Triage (Full Fetch for suspicious sender) ---
            ...
            for full_msg in mailbox.fetch(AND(uid=uids_to_fetch)):
                ...
                if full_msg.from_ in whitelist:
                    if is_promotional(full_msg):
                        ...
                        self.record.write(f"Whitelist Promo: Whitelisted sender {full_msg.from_} sent a promo. Moving to trash.")
                    continue

                if has_malware(full_msg):
                    categories["to_bin"].append(full_msg)
                    categories["to_blacklist"].append(full_msg)
                    self.record.write(f"Malware: Found dangerous attachment from {full_msg.from_}. Blacklisting.")
                    continue

                if has_suspicious_docs(full_msg):
                    categories["for_file_analysis"].append(full_msg)
                    self.record.write(f"Suspicious Document: Potential threat from {full_msg.from_}. Sending to BombSquad.")
                    continue

                if has_spy_pixel(full_msg):
                    categories["to_bin"].append(full_msg)
                    self.record.write(f"Spy Pixel: Found tracking pixel from {full_msg.from_}. Moving to trash.")
                    continue

                if is_promotional(full_msg):
                    categories["to_bin"].append(full_msg)
                    self.record.write(f"Promotion: Graylisted sender {full_msg.from_} sent a promo. Moving to trash.")
                    continue
                ...
                self.record.write(f"Safe: Email from {full_msg.from_} passed triage. Leaving in Inbox.")
    ...
    def _applying(self, mailbox, categories):
        ...
        # most complicated first (bomb_squad)
        ...
        if categories["to_unsubscribe"]:
            for msg in categories["to_unsubscribe"]:
                if unsubscribe_from(msg, self.smtp_data):
                    self.record.write(f"Unsubscribe: Successfully unsubscribed from {msg.from_}.")
                else:
                    self.record.write(f"Unsubscribe: Failed request for {msg.from_}.")
